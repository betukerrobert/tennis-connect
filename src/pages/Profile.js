...
function Profile() {
  ...
  const fetchProfile = async () => {
    ...
    if (error) { console.error('Error fetching profile:', error); } else { setProfile(data); setForm(data); }
    setLoading(false);
  };
  ...
  const fetchConnectionsCount = async () => {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return;
    const { count, error } = await supabase.from('connections').select('*', { count: 'exact', head: true }).eq('status', 'accepted').or(`sender_id.eq.${user.id},receiver_id.eq.${user.id}`);
  ...
  const fetchMatchStats = async () => {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return;
    const { data: matches } = await supabase.from('matches').select('id, match_date, result, court_name, status, sender_id, receiver_id').or(`sender_id.eq.${user.id},receiver_id.eq.${user.id}`).order('match_date', { ascending: false });
    if (!matches) return;
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase.from('profiles').update({
      ...
      coaching_specialisation: Array.isArray(form.coaching_specialisation) && form.coaching_specialisation.length > 0 ? form.coaching_specialisation : null,
    }).eq('id', profile.id);
  ...
  const handleAvatarUpload = async (e) => {
    ...
    } catch (err) { console.error('Upload error:', err.message); } finally { setUploading(false); }
  };
  ...
  // Helper: get specialisation as array regardless of storage format
  ...
  const toggleFormSpecialisation = (s) => {
    const current = getSpecialisationArray(form.coaching_specialisation);
    const updated = current.includes(s) ? current.filter(x => x !== s) : [...current, s];
    setForm({ ...form, coaching_specialisation: updated });
  };
  ...
  const formSpecialisationArray = getSpecialisationArray(form.coaching_specialisation);
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.profileHero}>
        ...
        {editing ? (
          <input style={styles.editInput} value={form.full_name || ''} onChange={e => setForm({ ...form, full_name: e.target.value })} placeholder="Your full name" />
        ) : (
        ...
        {editing ? (
          <input style={styles.editInput} value={form.location || ''} onChange={e => setForm({ ...form, location: e.target.value })} placeholder="Your location" />
        ) : (
      ...
      <div style={styles.section}>
        <button style={styles.matchesBtn} onClick={() => navigate('/matches')}>🎾 My Matches</button>
      </div>
      ...
      <div style={styles.section}>
        ...
        {editing ? (
          <textarea style={styles.editTextarea} value={form.bio || ''} onChange={e => setForm({ ...form, bio: e.target.value })} placeholder="Tell others about yourself..." rows={3} />
        ) : (
          <p style={styles.bio}>{profile.bio || 'No bio yet — tap Edit to add one!'}</p>
        )}
      </div>
        ...
        <div style={styles.detailsGrid}>
          {[
            ...
            <div key={d.key} style={styles.detailItem}>
              ...
              {editing ? (
                <input style={styles.detailEditInput} value={form[d.key] || ''} onChange={e => setForm({ ...form, [d.key]: e.target.value })} placeholder={d.placeholder} />
              ) : (
          ...
          <div style={styles.detailItem}>
            ...
            {editing ? (
              <select style={styles.detailEditSelect} value={form.preferred_surface || ''} onChange={e => setForm({ ...form, preferred_surface: e.target.value })}>
          ...
          <div style={styles.detailItem}>
            ...
            {editing ? (
              <input style={styles.detailEditInput} type="number" value={form.age || ''} onChange={e => setForm({ ...form, age: e.target.value })} placeholder="e.g. 28" min="10" max="99" />
            ) : (
      ...
      <div style={styles.section}>
        ...
        <div style={styles.extraCard}>
          <div style={styles.extraCardTop}>
            ...
            <div style={{ ...styles.toggle, backgroundColor: profile.open_to_sparring ? '#c8ff00' : '#e0e4ea' }} onClick={handleToggleSparring}>
          ...
          {profile.open_to_sparring && (
            <div style={styles.extraFields}>
              <div style={styles.extraFieldRow}>
                ...
                {editing ? (
                  <input style={styles.extraFieldInput} type="number" value={form.sparring_rate || ''} onChange={e => setForm({ ...form, sparring_rate: e.target.value })} placeholder="e.g. 15" min="0" />
                ) : (
                  <span
        ...
        <div style={{ ...styles.extraCard, marginTop: '10px' }}>
          <div style={styles.extraCardTop}>
            ...
            <div style={{ ...styles.toggle, backgroundColor: profile.also_coaches ? '#a855f7' : '#e0e4ea' }} onClick={handleToggleCoaching}>
          ...
          {profile.also_coaches && (
            <div style={styles.extraFields}>
              <div style={styles.extraFieldRow}>
                <span style={styles.extraFieldLabel}>Hourly Rate (€)</span>
                {editing ? (
                  <input style={styles.extraFieldInput} type="number" value={form.coaching_rate || ''} onChange={e => setForm({ ...form, coaching_rate: e.target.value })} placeholder="e.g. 40" min="0" />
                ) : (
                  <span
                ...
                {editing ? (
                  <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                    {specialisationOptions.map(s => {
                      const selected = formSpecialisationArray.includes(s);
                      return (
                        <span
                          key={s}
                          onClick={() => toggleFormSpecialisation(s)}
                          style={{ fontSize: '11px', padding: '4px 10px', borderRadius: '999px', cursor: 'pointer', fontWeight: '500', backgroundColor: selected ? 'rgba(168,85,247,0.15)' : '#f0f2f5', color: selected ? '#a855f7' : '#9aa0ac', border: `1px solid ${selected ? 'rgba(168,85,247,0.3)' : 'transparent'}` }}
                        >
                          {s}
                        </span>
                      );
                  ...
                  <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                    {specialisationArray.length > 0 ? (
                      specialisationArray.map(s => (
                        <span key={s} style={{ fontSize: '11px', padding: '3px 9px', borderRadius: '999px', backgroundColor: 'rgba(168,85,247,0.1)', color: '#a855f7', fontWeight: '500' }}>{s}</span>
                      ))
                    ) : (
                      <span
                        style={styles.extraFieldNotSet}
      ...
      {editing && (
        <div style={styles.section}>
          <button style={styles.cancelBtn} onClick={() => { setEditing(false); setForm(profile); }}>Cancel</button>
        </div>
      )}
...
const styles = {
  container: { fontFamily: "'Helvetica Neue', Arial, sans-serif", paddingBottom: '20px' },
  ...
  headerTitle: { fontSize: '24px', fontWeight: '300', color: '#0a1628', margin: '0', letterSpacing: '-0.5px' },
  editBtn: { color: '#0a1628', fontSize: '13px', cursor: 'pointer', fontWeight: '600', backgroundColor: '#f4f6f8', padding: '6px 14px', borderRadius: '999px', border: '1.5px solid #e0e4ea' },
  profileHero: { background: 'linear-gradient(135deg, #0a1628 0%, #1a2d4a 100%)', borderRadius: '20px', padding: '28px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', marginBottom: '16px' },
  avatarWrapper: { position: 'relative', cursor: 'pointer', marginBottom: '4px' },
  ...
  avatarOverlay: { position: 'absolute', bottom: '-4px', right: '-4px', backgroundColor: '#c8ff00', borderRadius: '50%', width: '24px', height: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px' },
  ...
  editInput: { backgroundColor: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '8px', padding: '8px 12px', fontSize: '14px', color: 'white', outline: 'none', width: '80%', textAlign: 'center' },
  statsRow: { backgroundColor: 'white', borderRadius: '14px', padding: '16px', display: 'flex', justifyContent: 'space-around', alignItems: 'center', boxShadow: '0 2px 10px rgba(10,22,40,0.06)', marginBottom: '16px' },
  statItem: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' },
  statValue: { fontSize: '20px', fontWeight: '600', color: '#0a1628', letterSpacing: '-0.3px' },
  ...
  matchesBtn: { width: '100%', padding: '14px', borderRadius: '12px', border: 'none', backgroundColor: '#0a1628', color: '#c8ff00', fontSize: '14px', fontWeight: '700', cursor: 'pointer', fontFamily: "'Helvetica Neue', Arial, sans-serif", letterSpacing: '0.2px' },
  ...
  editTextarea: { width: '100%', padding: '12px 14px', borderRadius: '12px', border: '1.5px solid #e0e4ea', fontSize: '13px', color: '#0a1628', outline: 'none', resize: 'none', fontFamily: "'Helvetica Neue', Arial, sans-serif", boxSizing: 'border-box' },
  ...
  detailEditInput: { border: 'none', borderBottom: '1.5px solid #e0e4ea', padding: '4px 0', fontSize: '13px', color: '#0a1628', outline: 'none', backgroundColor: 'transparent', fontFamily: "'Helvetica Neue', Arial, sans-serif" },
  detailEditSelect: { border: 'none', borderBottom: '1.5px solid #e0e4ea', padding: '4px 0', fontSize: '13px', color: '#0a1628', outline: 'none', backgroundColor: 'transparent', fontFamily: "'Helvetica Neue', Arial, sans-serif", width: '100%' },
  ...
  toggle: { width: '44px', height: '26px', borderRadius: '999px', cursor: 'pointer', position: 'relative', transition: 'background-color 0.2s', flexShrink: 0 },
  toggleKnob: { position: 'absolute', top: '3px', width: '20px', height: '20px', borderRadius: '50%', backgroundColor: 'white', boxShadow: '0 1px 4px rgba(0,0,0,0.2)', transition: 'transform 0.2s' },
  extraFields: { marginTop: '14px', paddingTop: '14px', borderTop: '1px solid #f0f2f5', display: 'flex', flexDirection: 'column', gap: '12px' },
  extraFieldRow: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  extraFieldLabel: { fontSize: '12px', color: '#5a6270', fontWeight: '500' },
  extraFieldValue: { fontSize: '13px', fontWeight: '600', color: '#0a1628' },
  extraFieldNotSet: { fontSize: '12px', fontWeight: '500', color: '#9aa0ac', cursor: 'pointer', textDecoration: 'underline', textDecorationStyle: 'dotted' },
  extraFieldInput: { border: 'none', borderBottom: '1.5px solid #e0e4ea', padding: '4px 8px', fontSize: '13px', color: '#0a1628', outline: 'none', backgroundColor: 'transparent', fontFamily: "'Helvetica Neue', Arial, sans-serif", width: '80px', textAlign: 'right' },
  cancelBtn: { width: '100%', backgroundColor: 'transparent', color: '#9aa0ac', padding: '13px', borderRadius: '12px', border: '1.5px solid #e0e4ea', fontSize: '13px', fontWeight: '500', cursor: 'pointer' },
  subscriptionCard: { backgroundColor: '#0a1628', borderRadius: '14px', padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  planLeft: { display: 'flex', alignItems: 'center', gap: '12px' },
  planIconBox: { fontSize: '22px' },
  planName: { margin: '0 0 2px 0', fontWeight: '600', fontSize: '14px', color: 'white' },
  planDesc: { margin: '0', fontSize: '11px', color: 'rgba(255,255,255,0.4)', fontWeight: '400' },
  upgradeBtn: { backgroundColor: '#c8ff00', color: '#0a1628', padding: '8px 16px', borderRadius: '999px', border: 'none', fontSize: '12px', fontWeight: '700', cursor: 'pointer' },
  logoutBtn: { width: '100%', backgroundColor: 'transparent', color: '#ef4444', padding: '13px', borderRadius: '12px', border: '1.5px solid #ef444440', fontSize: '13px', fontWeight: '500', cursor: 'pointer' },
};
